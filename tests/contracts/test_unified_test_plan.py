"""Static and state-machine contracts for the nine-round unified test runner."""
...
def test_unified_test_has_exact_nine_round_order() -> None:
    assert [
        (round_spec.number, round_spec.name) for round_spec in unified_test.ROUNDS
    ] == [
...
def test_first_compile_and_first_import_exist_only_in_round_one() -> None:
    commands = {
        round_spec.number: [
            step.argv for step in round_spec.steps if step.kind == "command"
        ]
    ...
    for number in range(2, 10):
        assert all(
            "cargo check" not in " ".join(command) for command in commands[number]
        )
        assert all(
            "maturin develop" not in " ".join(command) for command in commands[number]
        )
...
def test_each_round_preserves_plan_step_order() -> None:
    ...
    assert names[3][0] == "golden-consistency"
    ...
    assert names[8][0] == "five-wheel-and-sdist-evidence"
    assert names[9][-2:] == ["publish-workflow-dry-run-no-upload", "evidence-index"]


def test_round_two_filters_have_static_rust_test_targets() -> None:
    root = Path(unified_test.ROOT)
    round_two = next(
        round_spec for round_spec in unified_test.ROUNDS if round_spec.number == 2
    )
    ...
    for step_name, (source_root, test_filter) in expected.items():
        ...
        test_name = re.compile(
            rf"\bfn\s+[A-Za-z0-9_]*{re.escape(test_filter)}[A-Za-z0-9_]*\b"
        )
        matches = [
            path
...
def test_remote_requirements_are_evidence_gates_not_actions() -> None:
    ...
    assert gates == {"round6_windows_linux", "round8_artifacts"}
    for round_spec in unified_test.ROUNDS:
    ...
    for round_spec in unified_test.ROUNDS:
        for step in round_spec.steps:
            ...
            for prefix in forbidden:
                assert step.argv[: len(prefix)] != prefix, (
                    round_spec.number,
                    step.name,
                )
...
def test_candidate_check_requires_exact_head_and_clean_worktree(
...
def test_step_cleanliness_failure_is_recorded(
    ...
    def assert_candidate(
        _candidate: str, *, allow_editable_native: bool = False
    ) -> None:
        nonlocal checks
    ...
    with pytest.raises(RuntimeError, match="unexpected.txt"):
        unified_test.run_next_round("abc", artifact_dir=None)
...
def test_external_evidence_is_content_addressed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        unified_test.ExternalEvidenceRequired, match="changed or is missing"
    ):
        unified_test._check_gate(step, state)
