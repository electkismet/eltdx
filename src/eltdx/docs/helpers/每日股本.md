...
`DailyShareCapitalTable` 包含 `codes`、`rows`、`count`。每行 `DailyShareCapital` 包含 `full_code`、`trade_date`、`total_shares`、`circulating_shares`、`free_float_shares`、`finance_updated_date` 和 `share_source`。
