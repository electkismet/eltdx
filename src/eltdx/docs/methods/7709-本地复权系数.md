...
这个接口为用户已有的本地不复权 K 线提供前复权或后复权所需系数。它只根据 `0x000f` 返回的标签 `1` 权息事件计算仿射系数，不请求也不返回 K 线，不使用 `0x052d` 的服务端复权结果。

## 先判断是否需要这个接口

| 用户需求 | 使用方式 |
| --- | --- |
| 直接获取不复权 K 线 | `client.bars.get(..., adjust="none")` |
| 直接获取前复权 K 线 | `client.bars.get(..., adjust="qfq")` |
| 直接获取后复权 K 线 | `client.bars.get(..., adjust="hfq")` |
| 已有本地不复权 K 线，需要自行复权 | `client.corporate.adjustment_factors()` |

前三种情况都由 `0x052d` 直接返回 K 线，不需要先调用本接口。只有最后一种情况需要本地复权系数：用户把返回的 `scale + offset` 按日期应用到已有 OHLC，生成自己的前复权或后复权价格。
...
除离线复权本地行情库外，本接口也可用于公式审计以及独立验证服务端复权结果。
...
| --- | --- |
| `code` | 证券代码 |
| `anchor_date` | 前复权事件截止日期；默认包含全部标签 `1` 事件。这里按自然日期比较，不请求交易日历 |
