...
根据 `0x000f` 返回的标签 `1` 权息事件，生成每个除权事件日期对应的前复权和后复权仿射系数。将这些系数按日期应用到本地保存的不复权 OHLC，可以得到基于权息资料的本地复权结果。
...
普通前复权和后复权应直接使用 [`client.bars.get()`](7709-K线周期线.md) 的 `adjust="qfq"` 或 `"hfq"`，由 `0x052d` 主站计算。本接口只用于已有不复权 K 线的本地计算、公式审计和结果核验，不是服务端复权结果的逐值等价替代品。
...
!!! warning "默认系数不等于服务端复权结果"
    `start_date=None` 表示使用全部有日期的权息事件，不表示“从服务端第一根可用 K 线开始”。部分证券存在早于第一根可用 K 线的权息记录；若把默认返回的后复权系数直接应用到全部不复权 K 线，上市前事件也会进入累计基准并造成显著偏差。重建一段完整 K 线时，必须传入该序列第一根 K 线的日期作为 `start_date`。
...
| 路径 | 数据源 | 契约 |
| --- | --- | --- |
| `client.bars.get(..., adjust="qfq" / "hfq")` | `0x052d` 主站内部复权 | eltdx 对外提供的服务端复权结果，普通使用优先选择 |
| `client.corporate.adjustment_factors()` | `0x000f` 权息字段本地复合 | 用于本地计算和审计，不承诺逐值精确重建 `0x052d` |

在正确传入 K 线起止日期后，两条路径仍可能因为协议字段精度、累计精度和舍入顺序不同而出现约 `0.01～0.02` 元差异。这个小额差异属于两条数据路径的精度边界；未传 `start_date` 导致的上市前事件累计可能产生远大于该范围的偏差，不属于精度误差。
...
验证程序按日期对齐三组 K 线，对不复权 OHLC 应用本地系数，再分别比较 `server_qfq` 和 `server_hfq`。`0x052d` 只在验证程序中提供输入价格和参照结果，不参与系数计算；比较时应接受上述约 `0.01～0.02` 元的精度差异，而不是要求逐值完全相等。
