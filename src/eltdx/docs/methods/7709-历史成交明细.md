...
| 项目 | 内容 |
| --- | --- |
| 主要调用 · 单页 | `client.trades.history(code, trading_date, ..., batch_size=None)` |
| 完整分页 | `client.trades.all_history(code, trading_date, batch_size=None)` |
...
`history()` 和 `all_history()` 也接受代码列表。列表输入时底层仍逐只请求，客户端按 `batch_size` 并发，返回以规范化完整代码为键的字典；代码列表没有总数量上限，超出并发数的代码排队等待；传入单个字符串时返回类型不变。
