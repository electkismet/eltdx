...
| 项目 | 内容 |
| --- | --- |
| 主要调用 · 单页 | `client.trades.history(code, trading_date, ...)` |
| 完整分页 | `client.trades.all_history(code, trading_date, ...)` |
| 可选批量返回 | `client.trades.history_batch(code, trading_date, ...)` |
| 批量完整分页 | `client.trades.all_history_batch(code, trading_date, ...)` |
| 对象返回 · 单页 | `client.trades.history(code, trading_date, ...)` → `TradePage` |
| 对象返回 · 完整分页 | `client.trades.all_history(code, trading_date, ...)` → `TradePage` |
| 批量返回 · 单页 | `client.trades.history_batch(code, trading_date, ...)` → `TradeBatch` |
| 批量返回 · 完整分页 | `client.trades.all_history_batch(code, trading_date, ...)` → `TradeBatch` |
| 底层接口 | [`0x0fc6`](../COMMANDS_7709.md#cmd-0x0fc6) |

以上是同一个历史成交明细接口的四种 Python 调用方式，`code` 均支持单个股票或代码列表；传列表时返回 `{完整代码: 对应结果}`。原调用方式保持不变，批量返回方式省去下载时逐条创建 `TradeTick` 的开销，适合按列读取和筛选。
