...
`raw_members`、`display_members` 和 `excluded_members` 的每项至少包含 `market`、`code`；排除项额外包含 `reason`，当前值为 `absent_from_current_security_list`。`rows` 中每项为 `BoardQuoteRow`，其中 `full_code` 是带市场前缀的证券代码。
