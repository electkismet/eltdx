...
!!! warning "历史版本文档"
    本页归档的是 `v1.0.0` 发布时相对 `v0.5.1` 的变化，文中的 `client.get_*()` 示例只适用于当时的 1.x API。当前 `v2.0.3` 已移除这些入口，请按 [迁移到 eltdx 2.0](MIGRATION_FROM_OLD.md) 使用模块化 API。
...
以下示例保留当时发布内容，不适用于当前 `v2.0.3`。当前代码请查看 [迁移到 eltdx 2.0](MIGRATION_FROM_OLD.md)。
