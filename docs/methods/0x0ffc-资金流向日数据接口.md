...
按证券代码读取 7709 主站最近的日资金流向记录。接口对应客户端页面的 `ASK_OneZJLX` 数据服务，通常返回最近 5 个交易日；它不是实时推送接口，也不负责计算 K 线复权。
...
`route=0x7e` 和 `channel=0x2d` 是该服务的内部封装字段，由 Rust 协议层自动写入。用户只需要传证券代码，不需要传路由、通道或序号。
...
该接口只读取资金流向数据，不替代 `0x052d` K 线、`0x000f` 股本变迁，也不向用户暴露内部 `route` 字段。
