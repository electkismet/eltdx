...
| 项目 | 内容 |
| --- | --- |
| 主要调用 | `client.bars.get(code, ...)` |
| | 底层接口 | [`0x052d`](../COMMANDS_7709.md#cmd-0x052d) |
| 返回模型 | `KlineSeries` |

`all_pages=False`（默认）只取一页；`all_pages=True` 自动请求到主站返回空页，并把结果合并为一个 `KlineSeries`。不再提供单独的 `client.bars.all()`。
...
`client.bars.get()` 使用 `code` 位置参数，周期通过 `period=` 指定。

## 真实返回样本

Python 实际返回 `KlineSeries`。下面从 `to_jsonable(day)` 中保留系列信息和两根日 K；`bars` 完整列表及原始十六进制未全部展开。

??? return-sample "真实返回 JSON · KlineSeries（日 K，2 条节选）"
    <div class="return-sample-meta">
      <div><span>采样标的</span><code>sz000001</code></div>
      <div><span>数据日期</span><code>2026-08-13 至 2026-08-14</code></div>
      <div><span>返回类型</span><code>KlineSeries</code></div>
    </div>
    <p class="return-sample-note">真实采样；请求参数为 period="day"、count=3、adjust=None。</p>
...
| `all_pages` | `False` 取一页；`True` 自动翻页并合并 |
| `page_size` | 自动分页时的每页条数，默认及最大值均为 `800` |
| `max_pages` | 自动分页最大页数，默认 `200`；`None` 表示不设 SDK 页数上限 |
...
`adjust="none" / "qfq" / "hfq"` 的结果都由 `0x052d` 主站直接返回。用户只要直接获取不复权、前复权或后复权 K 线，就使用本接口，不需要先计算本地系数。只有已经持有本地不复权 K 线并希望自行复权时，才使用 [`client.corporate.adjustment_factors()`](7709-本地复权系数.md)。
