...
| 项目 | 内容 |
| --- | --- |
| 主要调用 · 分页 | `client.codes.list(market, ...)` |
| 主要调用 · 全量 | `client.codes.all(market, ...)` |
...
with TdxClient(timeout=3) as client:
    ...
    all_a_share = client.codes.all_a_shares()
```
...
with TdxClient(timeout=3) as client:
    sh_codes = client.codes.all("sh")
    sz_codes = client.codes.all("sz")
    bj_codes = client.codes.all("bj")
```
...
| `refresh` | 仅 `client.codes.all()` 支持；跳过缓存重新拉取 |
...
| 想取什么 | 调用 |
| --- | --- |
| 深市代码表 | `client.codes.all("sz")` 或 `client.codes.all("sz")` |
| 沪市代码表 | `client.codes.all("sh")` 或 `client.codes.all("sh")` |
| 北交所代码表 | `client.codes.all("bj")` 或 `client.codes.all("bj")` |
...
`client.codes.all(market)` 每次按协议分页拉取全量代码表。
...
| 第一页 | `start=0` |
| 下一页 | `start += 当前页实际返回数量` |
| 停止条件 | 当前页返回数量 `< page_size`；自动分页按实际返回数量推进偏移 |
| 默认页大小 | `1600` |
