...
| `board_level` | `lbts` | **该股票实际连板数**；只对 `ztlb=1` 的涨停行提供，炸板/跌停行规范为 `None` |
| `ladder_days` | `lbts` | `board_level` 的兼容别名 |
...
| `seal_amount` | `fde` | 封单金额；炸板行通常为 `0`，跌停行不应当作涨停封单解读 |
| `limit_time` | `ztsj` | 涨跌停/触及涨跌停时间，格式通常为 `HH:MM:SS` |
| `broken_count` | `kbcs` | 开板次数 |
