...
| `eltdx_auction_data` | 不传日期汇总当日竞价过程、09:25 正式撮合和前收盘参考价；传日期时用 `0x056a` 查询历史竞价过程，并用 `0x0fc6` 取得正式撮合和前收盘参考价 |
...
F10 工具保留服务端返回的原始字段名，同时在每个 `result_sets` 项中增加：

- `field_labels`：按 Entry、请求分区和响应模板解析的已确认中文字段名；财务报表覆盖 `zcfzb`、`lyb` 和 `xjllb`，并按 `nhytype` 区分通用企业、银行、券商和保险模板；
- `unmapped_fields`：当前分区尚未确认含义、或不能使用单一标签安全解释的原始字段名；
- `field_notes`：仅为当前结果中需要额外解释的字段返回口径说明，例如分组标题或动态扩展字段。

字段映射不会跨 Entry 或财务行业模板复用。例如通用企业资产负债表中的 `T039` 是“资产总计”，银行、券商和保险模板中的 `T039` 是“固定资产”，公司研报中的 `T039` 又是“研报标题”。财务工具读取元数据表的 `nhytype`（`0` 通用、`1` 银行、`2` 券商、`3` 保险）选择映射；缺少或遇到未知模板时不猜测。银行历史现金流量表的 `T088` 是随证券和报告期变化的动态扩展调节项，因此保留在 `unmapped_fields` 并通过 `field_notes` 解释，而不是强行指定统一中文名。原始 `columns`、`rows` 和 `row_cells` 不变，现有调用方可以继续按原字段读取。
