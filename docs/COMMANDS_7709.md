...
| <a id="cmd-0x056a"></a>`0x056a` | `0x056a-集合竞价明细接口.md`      | 当日 / 历史集合竞价过程快照 | `client.auctions.series(code, date=None)` | 已接入          |
...
说明：`0x0547` 已完成请求构造、响应解析、单次刷新和未配对推送队列。实盘主站对单次超过 100 个代码的请求只返回前 100 条，因此构包层将单次上限固定为 100，避免静默截断。`0x06b9` 的协议层返回单个 `FileContentChunk`；整文件下载和 `zhb.zip` 内容解析位于上层封装。
...
| [`0x0547`](methods/7709-增量刷新推送队列.md) | 行情增量刷新 / 实时五档 | 最多 100 个代码 + 每只代码的游标 | 首次刷新返回五档记录，之后推送增量更新 | 首次游标 `0` 建立刷新基线，之后持续刷新；配合推送队列 | 实时五档和连续更新行情 |
| [`0x053e`](methods/7709-旧版批量行情.md) | 旧版完整快照 | 显式代码列表，无游标 | 买一到买五 / 卖一到卖五 | 交易状态、尾部指标和旧版协议原始字段 | 兼容旧协议或研究旧版状态字段 |

常规基础快照使用 `client.quotes.get_snapshots(codes)`；需要实时完整五档时使用 `client.helpers.full_quotes(codes)` 或 `client.quotes.get_depth(codes)`，底层组合或调用 `0x0547`。`client.quotes.legacy(codes)` 仅用于旧版完整快照和原始状态字段。
